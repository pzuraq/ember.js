...
function pushUniqueListener(destination: any[], listener: Listener) {
  for (let destinationIndex = 0; destinationIndex < destination.length; destinationIndex += 3) {
    if (destination[destinationIndex] === listener.target && destination[destinationIndex + 1] === listener.method) {
      return;
    }
  }
  destination.push(listener.target, listener.method, listener.kind === ListenerKind.ONCE);
}
